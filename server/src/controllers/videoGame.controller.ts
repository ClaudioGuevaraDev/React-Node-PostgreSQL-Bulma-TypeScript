...
export const updateVideoGame: RequestHandler = async (req, res) => {
    ...
    if (result.rowCount === 0) return res.status(404).json({ message: 'No se encontró el vidoejuego.' })
...
export const updatedImageVideoGame: RequestHandler = async (req, res) => {
    ...
    if (imageFound.rowCount === 0) return res.status(404).json({ message: 'No se encontró el videojuego.' })
